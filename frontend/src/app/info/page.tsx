...
export default function InfoPage() {
    return (
        <div className="max-w-6xl mx-auto px-4 py-12 space-y-12">
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                    {actionLinks.map(({ href, label, icon: Icon }) => (
                        <Link
                            key={href}
                            href={href}
                            className="inline-flex items-center justify-center gap-2 rounded-full border border-gray-200 dark:border-gray-700 bg-white/80 dark:bg-gray-900/40 px-5 py-2 text-sm font-medium text-gray-900 dark:text-white shadow-sm hover:border-blue-500 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                        >
                    ...
                    {[
                        {
                            ...
                            desc: "Controllers chamam os use cases (ex.: SendNotificationUseCase). Cada ação publica um `notifications.sent`, `friend.request.*` ou evento equivalente.",
                        },
                        {
                            title: "RabbitMQ encadeia o trabalho",
                            desc: "O `GenericEventsConsumer` lê da fila `events`, enriquece com `eventId` e envia para `notifications.realtimes` ou filas temáticas como `events.invites`.",
                        },
                        {
                            title: "Redis mantém a confirmação",
                            desc: "O `RealtimeNotificationsConsumer` guarda o payload no Redis até o frontend enviar `notifications.delivered`. Se o usuário desligou realtime, o worker salva direto e marca como entregue.",
                        },
                        {
                            title: "Frontend aplica preferências",
                            desc: "Hooks (`useNotificationPreferences`) determinam se mostra popup, badge ou apenas atualiza a lista. Mesmo sem alerta visual, a notificação já está persistida.",
                        },
                    ].map((step, index) => (
            ...
            <section className="grid gap-6 md:grid-cols-2">
                <article className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-900/40 p-6 shadow-sm space-y-3">
                    <div className="flex items-center gap-3">
                ...
                <article className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-blue-50/80 dark:bg-blue-900/20 p-6 shadow-sm space-y-3">
