...
describe('Realtime client', () => {
  ...
  it('subscribe attaches and detaches listener', async () => {
    ...
    const off = subscribe('companys.updated', handler);
    off();
    expect(typeof off).toBe('function');
  });

  it('handles notifications.read event', async () => {
    const s = getRealtimeSocket();
    const handler = jest.fn();
    subscribe('notifications.read', handler);
    // simulate emit
    (s as any).emit('notifications.read', { notificationId: 1 });
