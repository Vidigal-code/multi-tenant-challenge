...
describe('Invite Flow Integration', () => {
    ...
    it('complete flow: view invites -> accept -> verify', async () => {
        ...
        await waitFor(() => {
            expect(httpMock.post).toHaveBeenCalledWith('/auth/accept-invite', { token: 'token123' });
        }, { timeout: 5000 });
    });

    it('view created invites -> delete invite', async () => {
        httpMock.get.mockImplementation((url: string) => {
            if (url.includes('/auth/profile')) {
                return Promise.resolve({
                    data: {
    ...
    it('accept invite via link', async () => {
        httpMock.get.mockImplementation((url: string) => {
            if (url.includes('/auth/profile')) {
                return Promise.resolve({
                    data: {
    ...
    it('reject invite via link', async () => {
        httpMock.get.mockImplementation((url: string) => {
            if (url.includes('/auth/profile')) {
                return Promise.resolve({
                    data: {
