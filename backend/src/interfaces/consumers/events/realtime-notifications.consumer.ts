...
class RealtimeNotificationsConsumer extends BaseDeliveryAwareConsumer<any> {
  ...
  protected async processWithDelivery(
    payload: any,
    messageId: string,
  ): Promise<{ confirmed: boolean; saved?: boolean; error?: string }> {
    try {
      this.logger
        .rabbitmq(`Processing realtime notification with delivery confirmation: 
      ...
      if (eventId === "FRIEND_REQUEST_SENT") {
        this.logger.rabbitmq(
          `FRIEND_REQUEST_SENT payload keys: ${Object.keys(payload).join(", ")}`,
        );
        this.logger.rabbitmq(
          `FRIEND_REQUEST_SENT friendshipId: ${payload?.friendshipId || "NOT FOUND"}`,
        );
        this.logger.rabbitmq(
          `FRIEND_REQUEST_SENT additionalData: ${JSON.stringify(payload?.additionalData || {})}`,
        );
      }
      ...
      if (
        ...
        this.logger.rabbitmq(
          `Using receiver.id for ${eventId}: ${payload.receiver.id}`,
        );
      } else {
        const userId =
          payload?.userId || payload?.recipientUserId || payload?.invitedUserId;
        if (userId) {
          userIds.push(userId);
          this.logger.rabbitmq(
            `Using fallback userId for ${eventId}: ${userId}`,
          );
          ...
          if (user) {
            ...
            this.logger.rabbitmq(
              `Found user by email for INVITE_CREATED: ${user.id} (${payload.receiverEmail})`,
            );
          } else {
            this.logger.rabbitmq(
              `User not found by email for INVITE_CREATED: ${payload.receiverEmail}`,
            );
          }
          ...
          if (user) {
            userIds.push(user.id);
            this.logger.rabbitmq(
              `Found user by email for INVITE_CREATED: ${user.id} (${payload.invitedEmail})`,
            );
          } else {
            this.logger.rabbitmq(
              `User not found by email for INVITE_CREATED: ${payload.invitedEmail}`,
            );
          }
        } else if (eventId === "FRIEND_REQUEST_SENT" && payload?.addresseeId) {
          userIds.push(payload.addresseeId);
          this.logger.rabbitmq(
            `Found recipient for FRIEND_REQUEST_SENT: ${payload.addresseeId}`,
          );
        } else if (
          ...
          this.logger.rabbitmq(
            `Found recipient for FRIEND_REQUEST_ACCEPTED: ${payload.requesterId}`,
          );
        } else if (
          ...
          userIds.push(payload.requesterId);
          this.logger.rabbitmq(
            `Found recipient for FRIEND_REQUEST_REJECTED: ${payload.requesterId}`,
          );
        } else if (eventId === "FRIEND_REMOVED") {
          const { requesterId, addresseeId, userId } = payload;
          if (userId && requesterId && addresseeId) {
            const notifyUserId =
              userId === requesterId ? addresseeId : requesterId;
            if (notifyUserId) {
              userIds.push(notifyUserId);
              this.logger.rabbitmq(
                `Found recipient for FRIEND_REMOVED: ${notifyUserId}`,
              );
            }
          } else if (payload?.addresseeId) {
            userIds.push(payload.addresseeId);
            this.logger.rabbitmq(
              `Found recipient for FRIEND_REMOVED: ${payload.addresseeId}`,
            );
          } else if (payload?.requesterId) {
            userIds.push(payload.requesterId);
            this.logger.rabbitmq(
              `Found recipient for FRIEND_REMOVED: ${payload.requesterId}`,
            );
          }
        }
      }
      ...
      this.logger.rabbitmq(
        `Preparing to emit notifications to ${userIds.length} user(s): ${userIds.join(", ")}`,
      );

      if (userIds.length === 0) {
        this.logger.rabbitmq(
          `Warning: No user IDs found in payload, creating notification without WebSocket delivery`,
        );
      ...
      for (const userId of userIds) {
        ...
        this.logger.rabbitmq(
          `User ${userId} preferences: realtimeEnabled=${prefs.realtimeEnabled}, realtimePopups=${prefs.realtimePopups}`,
        );
        ...
        await this.deliveryConfirmation.storePendingDelivery(
          userMessageId,
          notificationPayload,
          {
            userId,
        ...
        this.gateway.emitToUser(
          userId,
          this.RT_EVENT.NOTIFICATION_CREATED,
          notificationPayload,
        );
      ...
      if (companyId) {
        ...
        this.logger.rabbitmq(
          `WebSocket event emitted to company: ${companyId}, messageId: ${messageId}`,
        );
      }

      if (userMessageIds.size === 0) {
        this.logger.rabbitmq(
          `No users require confirmation, all notifications saved`,
        );
        return { confirmed: true, saved: true, error: undefined };
      }
      ...
      while (
        ...
        confirmedUserIds.length < userMessageIds.size
      ) {
        for (const [userId, userMessageId] of userMessageIds.entries()) {
          if (confirmedUserIds.includes(userId)) continue;

          const isPending =
            await this.deliveryConfirmation.isPending(userMessageId);
          if (!isPending) {
            const confirmedData =
              await this.deliveryConfirmation.confirmDelivery(userMessageId);
            if (confirmedData) {
              this.logger.rabbitmq(
                `Delivery confirmed for user ${userId}: messageId=${userMessageId}`,
              );
              try {
                ...
                const notificationPayload = {
                  ...
                  recipientUserId:
                    payload?.receiver?.id ||
                    storedPayload?.recipientUserId ||
                    userId,
                  senderUserId:
                    payload?.sender?.id ||
                    payload?.senderUserId ||
                    storedPayload?.senderUserId,
                  companyId:
                    payload?.companyId ||
                    payload?.company?.id ||
                  ...
                  invitedEmail:
                    ...
                    payload?.receiver?.email ||
                    payload?.email ||
                    storedPayload?.invitedEmail ||
                    storedPayload?.receiverEmail,
                ...
                this.logger.rabbitmq(
                  `Notification payload full: ${JSON.stringify(notificationPayload, null, 2)}`,
                );
                await this.notificationCreator.createNotificationForEvent(
                  eventName,
                  notificationPayload,
                ...
                this.logger.rabbitmq(
                  `Notification saved in database for user ${userId}: eventName=${eventName}`,
                );
                confirmedUserIds.push(userId);
              } catch (error: any) {
                this.logger.error(
                  `Error creating notification for user ${userId}: ${error?.message || String(error)}`,
                );
                this.logger.error(
      ...
      for (const [userId, userMessageId] of userMessageIds.entries()) {
        if (!confirmedUserIds.includes(userId)) {
          ...
          try {
            ...
            this.logger.rabbitmq(
              `Notification saved in database after timeout for user ${userId}: eventName=${eventName}`,
            );
            confirmedUserIds.push(userId);
          } catch (error: any) {
            this.logger.error(
              `Error saving notification after timeout for user ${userId}: ${error?.message || String(error)}`,
            );
            this.logger.error(
              `Error stack: ${error?.stack || "No stack trace"}`,
            );
          ...
          this.logger.rabbitmq(
            `Pending delivery cleaned up after timeout for user ${userId}: messageId=${userMessageId}`,
          );
        }
      }
      ...
      return {
        ...
        error: allConfirmed
          ? undefined
          : `Some deliveries timed out, but all notifications saved: ${savedCount}/${userMessageIds.size}`,
      };
    } catch (error: any) {
      const errorMsg = error?.message || String(error);
      this.logger.error(
        `Failed to process realtime notification: messageId=${messageId}, error=${errorMsg}`,
      );
...
async function handleBootstrapError(
  ...
  logger.error(
    `Failed to start realtime notifications consumer: ${errorMessage}`,
  );
...
async function initializeApplication(logger: LoggerService): Promise<any> {
  ...
  logger.default(
    `Worker HTTP server listening on port ${workerPort} (for WebSocket initialization)`,
  );
...
function getRequiredServices(app: any): {
  ...
  } = require("@application/services/notification-creator.service");
  const {
    EventsGateway,
    RT_EVENT,
  } = require("../../../realtime/events.gateway");
  const {
    USER_REPOSITORY,
  } = require("@domain/repositories/users/user.repository");
  const {
    NOTIFICATION_REPOSITORY,
  } = require("@domain/repositories/notifications/notification.repository");
  const {
    DeliveryConfirmationService,
  } = require("@infrastructure/messaging/services/delivery-confirmation.service");
...
async function startConsumer(
  ...
  const consumer = new RealtimeNotificationsConsumer(
    rabbit,
    config,
...
async function bootstrap(): Promise<void> {
  ...
  try {
    ...
    context.stopAutoCleanup =
      services.deliveryConfirmation.startAutoCleanup(60000);
