...
class GenericEventsConsumer extends BaseResilientConsumer<any> {
    ...
    private resolveEventId(eventName: string): string {
        return this.eventIdMapping[eventName] || eventName.toUpperCase().replace(/\./g, '_');
    }
     ...
     */
    private async forwardToRealtimeQueue(enrichedPayload: any, eventName: string, eventId: string): Promise<void> {
