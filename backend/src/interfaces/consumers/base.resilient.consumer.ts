...
export abstract class BaseResilientConsumer<T = any> {
    ...
    private async handleMessage(channel: any, msg: any): Promise<void> {
        ...
        if (!parseResult.success) {
            this.rejectMessageToDlq(channel, msg, parseResult.error || 'Invalid message format');
            return;
        }
     ...
     */
    private parseMessage(msg: any): { success: boolean; payload?: T; error?: string } {
    ...
    private async checkDuplicate(dedupKey: string | null): Promise<boolean> {
        ...
        if (exists) {
            ...
            this.logger.rabbitmq(`Dedup: skipping duplicate message, key: ${dedupKey}, ttl: ${ttl}s`);
            return true;
        }
     ...
     */
    private async processMessage(channel: any, msg: any, payload: T, dedupKey: string | null): Promise<void> {
        try {
     ...
     */
    private async handleProcessingError(channel: any, msg: any, error: any): Promise<void> {
