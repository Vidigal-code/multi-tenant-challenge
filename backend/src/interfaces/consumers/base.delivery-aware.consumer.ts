...
import {
  BaseResilientConsumer,
  ResilientConsumerOptions,
} from "./base.resilient.consumer";
 ...
 */
export abstract class BaseDeliveryAwareConsumer<
  T = any,
> extends BaseResilientConsumer<T> {
  ...
  constructor(
    ...
    this.confirmationTimeout =
      parseInt(process.env.DELIVERY_CONFIRMATION_TTL || "60", 10) * 1000;
  }
  ...
  protected async process(payload: T): Promise<void> {
    ...
    this.logger.rabbitmq(
      `Processing message with delivery confirmation: messageId=${messageId}`,
    );
    ...
    try {
      ...
      if (result.confirmed) {
        this.logger.rabbitmq(
          `Message confirmed and processed: messageId=${messageId}, saved=${result.saved || false}`,
        );
        await this.deliveryConfirmation.removePendingDelivery(messageId);
      } else {
        const errorMsg =
          result.error || "Delivery confirmation timeout or failed";
        this.logger.rabbitmq(
          `Message delivery failed: messageId=${messageId}, error=${errorMsg}`,
        );
      ...
      this.logger.error(
        `Error processing message with delivery confirmation: messageId=${messageId}, error=${errorMsg}`,
      );
  ...
  protected async waitForConfirmation(
    ...
    this.logger.rabbitmq(
      `Waiting for delivery confirmation: messageId=${messageId}, timeout=${timeoutMs}ms`,
    );
    ...
    while (Date.now() - startTime < timeoutMs) {
      ...
      if (!isPending) {
        const confirmed =
          await this.deliveryConfirmation.confirmDelivery(messageId);
        if (confirmed) {
    ...
    this.logger.rabbitmq(
      `Delivery confirmation timeout: messageId=${messageId}, elapsed=${Date.now() - startTime}ms`,
    );
    return false;
  }
   ...
   */
  protected async getPendingDelivery(
    messageId: string,
  ): Promise<{ payload: any; metadata: any; createdAt: number } | null> {
