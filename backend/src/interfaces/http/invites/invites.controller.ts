...
export class InvitesController {
  ...
  async list(
    ...
    const invitesWithCompany = await Promise.all(
      pendingInvites.map(async (i) => {
        ...
        const inviter = i.inviterId
          ? await this.users.findById(i.inviterId)
          : null;
        return {
          id: i.id,
  ...
  async getByCode(
    ...
    if (!invite) {
      this.logger.default(
        `Get invite failed: invite not found - code: ${inviteCode}, user: ${user.sub}`,
      );
    ...
    const inviter = invite.inviterId
      ? await this.users.findById(invite.inviterId)
      : null;
    ...
    const isRecipient =
      currentUser &&
      invite.email.toString().toLowerCase() ===
  ...
  async acceptByCode(
    ...
    if (!invite) {
      this.logger.default(
        `Accept invite failed: invite not found - code: ${inviteCode}, user: ${user.sub}`,
      );
      throw new ApplicationError("INVITE_NOT_FOUND");
    }
    if (!invite.isPending()) {
      this.logger.default(
        `Accept invite failed: invite already used - invite: ${invite.id}, user: ${user.sub}`,
      );
    ...
    if (invite.isExpired()) {
      ...
      this.logger.default(
        `Accept invite failed: invite expired - invite: ${invite.id}, user: ${user.sub}`,
      );
    ...
    if (!currentUser) {
      this.logger.default(
        `Accept invite failed: user not found - user: ${user.sub}`,
      );
    ...
    const existingMembership = await this.memberships.findByUserAndCompany(
      user.sub,
      invite.companyId,
    );
  ...
  async rejectByCode(
    ...
    if (!invite) {
      this.logger.default(
        `Reject invite failed: invite not found - code: ${inviteCode}, user: ${user.sub}`,
      );
      throw new ApplicationError(ErrorCode.INVITE_NOT_FOUND);
    }
    if (!invite.isPending()) {
      this.logger.default(
        `Reject invite failed: invite already used - invite: ${invite.id}, user: ${user.sub}`,
      );
      ...
      if (inviterUser) {
        await this.notifications.create({
          ...
          meta: {
            ...
            rejectedByEmail:
              rejectedUser?.email?.toString() || user.email || null,
  ...
  @UseGuards(JwtAuthGuard)
  @ApiOperation({
    summary: "Delete an invites (sender) or reject/delete received invites",
  })
  ...
  async deleteInvite(
    ...
    if (!invite) {
      this.logger.default(
        `Delete invite failed: invite not found - invite: ${inviteId}, user: ${user.sub}`,
      );
      throw new ApplicationError("INVITE_NOT_FOUND");
    }
    ...
    if (!isInviter && !isRecipient) {
      this.logger
        .default(`Delete invite failed: forbidden action - invite: ${inviteId}, user:
  ...
  @ApiBody({
    schema: {
      properties: { inviteIds: { type: "array", items: { type: "string" } } },
    },
  })
  ...
  async deleteInvites(
    ...
    for (const inviteId of inviteIds) {
      ...
      if (invite.inviterId !== user.sub) {
        this.logger
          .default(`Delete multiple invites failed: forbidden action - invite: ${inviteId}, user:
