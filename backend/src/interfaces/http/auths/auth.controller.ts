...
export class AuthController {
  ...
  async profile(@CurrentUser() user: any) {
    ...
    if (!dbUser) {
      this.logger.default(
        `Profile not found in database for user: ${user.sub}`,
      );
      return {
        id: user.sub,
        email: user.email,
  ...
  @ApiResponse({
    status: 400,
    description: "Validation errors or email already used",
    type: ErrorResponse,
  })
  @ApiBody({
    schema: {
  ...
  async signup(
    ...
    this.logger.default(
      `User created successfully: ${user.id}, email: ${body.email}`,
    );
    return user.toJSON();
  }
  ...
  async login(
    ...
    this.attachCookie(res, token);
    this.logger.default(
      `Login successful: user ${user.id}, email: ${body.email}`,
    );
    return user.toJSON();
  }
  ...
  @Post("accept-invites")
  @ApiOperation({
    summary: "Accept an invites and create/link users to companys",
  })
  @ApiResponse({ status: 200, description: "Invite accepted successfully" })
  @ApiResponse({
    status: 400,
    description: "Invite expired/used/not found or missing data",
    type: ErrorResponse,
  })
  @ApiBody({
    schema: {
  ...
  async updateProfile(
    ...
    this.logger.default(
      `Profile update request - user: ${user.sub}, has name: ${!!dto.name}, has email: ${!!dto.email}, has newPassword: ${!!dto.newPassword}, has notificationPreferences: ${dto.notificationPreferences !== undefined && dto.notificationPreferences !== null}`,
    );
    ...
    if (
      ...
      this.logger.default(
        `Profile update failed: no fields to update - user: ${user.sub}`,
      );
    ...
    if ((dto.email || dto.newPassword) && !dto.currentPassword) {
      this.logger.default(
        `Profile update failed: current password required - user: ${user.sub}`,
      );
    ...
    if (dto.newPassword) {
      ...
      if (!dbUser) {
        this.logger.default(
          `Profile update failed: user not found - user: ${user.sub}`,
        );
      ...
      if (!ok) {
        this.logger.default(
          `Profile update failed: invalid current password - user: ${user.sub}`,
        );
    ...
    if (dto.email) {
      ...
      if (existing && existing.id !== user.sub) {
        this.logger.default(
          `Profile update failed: email already in use - user: ${user.sub}, email: ${dto.email}`,
        );
    ...
    if (dto.notificationPreferences !== undefined) {
      ...
      this.logger.default(
        `Updating notification preferences - user: ${user.sub}, received: ${JSON.stringify(dto.notificationPreferences)}, current: ${JSON.stringify(currentPrefs)}, merged: ${JSON.stringify(mergedNotificationPreferences)}`,
      );
    }
  ...
  @UseGuards(JwtAuthGuard)
  @ApiOperation({
    summary: "List companies where users is primary owner (creator)",
  })
  @ApiResponse({
    status: 200,
    description: "Primary owner companies listed",
    type: PrimaryOwnerCompaniesResponseDto,
  })
  async getPrimaryOwnerCompanies(
    @CurrentUser() user: any,
    @Query("page") page = "1",
  ...
  @ApiOperation({
    summary:
      "List companies where user is a member (ADMIN or MEMBER) but not primary owner",
  })
  @ApiResponse({
    status: 200,
    description: "Member companies listed",
    type: MemberCompaniesResponseDto,
  })
  async getMemberCompanies(
    @CurrentUser() user: any,
    @Query("page") page = "1",
    @Query("pageSize") pageSize = "10",
  ...
  @ApiOperation({
    summary:
      "Permanently delete users account. Automatically deletes all companies where user is primary owner and removes user from all companies where they are ADMIN or MEMBER.",
  })
