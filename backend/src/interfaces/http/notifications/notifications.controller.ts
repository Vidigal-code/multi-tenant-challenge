...
export class NotificationsController {
  ...
  @Post()
  @ApiOperation({
    summary: "Send a global message to companys members or friends",
  })
  @ApiResponse({
    status: 201,
  ...
  async create(
    ...
    this.logger.default(
      `POST /notifications - user: ${user.sub}, company: ${body.companyId}, recipients: ${body.recipientsEmails?.length || "all"}`,
    );
    ...
    this.logger.default(
      `Notification sent - ${result.notifications.length} notifications created`,
    );
    return {
  ...
  @ApiOperation({
    ...
    description:
      "Marks a notifications as read and publishes a realtimes notifications.read event",
  })
  @ApiResponse({
    status: 200,
    description: "Notification marked as read",
    type: NotificationReadPayloadDto,
  })
  @ApiResponse({
    status: 404,
    description: "Notification not found or not owned by users",
    type: ErrorResponse,
  })
  @ApiResponse({
    status: 403,
  ...
  async delete(@CurrentUser() user: any, @Param("id") id: string) {
    return this.deleteNotification.execute({
      notificationId: id,
      userId: user.sub,
    });
  }
  ...
  @ApiOperation({
    ...
    description:
      "Reply to a notifications and send the response back to the original sender only",
  })
  @ApiResponse({ status: 201, description: "Reply sent" })
  @ApiResponse({
    status: 404,
    description: "Notification not found or not belongs to users",
    type: ErrorResponse,
  })
  @ApiResponse({
    status: 403,
