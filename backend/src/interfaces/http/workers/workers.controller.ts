...
export class WorkersController {
  ...
  constructor(
    ...
    this.overloadThreshold = parseInt(
      process.env.WORKER_OVERLOAD_THRESHOLD || "1000",
      10,
    );
    this.workerCapacitySharingFactor =
      config.get<number>("app.worker.capacitySharingFactor") || 256;
  }
  ...
  async getWorkerCount(
    ...
    if (method === "pending") {
      estimatedCount = Math.max(
        1,
        Math.ceil(pendingCount / this.workerCapacitySharingFactor),
      );
    } else if (method === "load") {
      ...
      estimatedCount = Math.max(
        1,
        Math.ceil(
          (pendingCount / this.workerCapacitySharingFactor +
  ...
  private async getWorkerTypeStatus(workerType: string): Promise<any> {
    try {
      ...
      this.logger.error(
        `Error getting worker status for ${workerType}: ${error?.message || String(error)}`,
      );
      return {
        type: workerType,
        error: error?.message || String(error),
