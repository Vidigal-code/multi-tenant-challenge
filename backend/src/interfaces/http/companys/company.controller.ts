...
export class CompanyController {
  ...
  async create(
    ...
    this.logger.default(
      `POST /company - user: ${user.sub}, name: ${body.name}`,
    );
    ...
    this.logger.default(
      `Company created successfully: ${company.id}, name: ${body.name}, owner: ${user.sub}`,
    );
  ...
  async list(
    ...
    this.logger.default(
      `GET /company - user: ${user.sub}, page: ${page}, size: ${pageSize}`,
    );
    ...
    this.logger.default(
      `Companies listed: ${result.total} found for user: ${user.sub}`,
    );
    return result;
  }
  ...
  @ApiResponse({
    status: 403,
    description: "User is not a member of companys",
    type: ErrorResponse,
  })
  async select(
    @CurrentUser() user: any,
  ...
  @Get(":id")
  @ApiOperation({
    summary: "Get companys details by id (public or active companys)",
  })
  ...
  @ApiOperation({
    summary:
      "Get public companys information (no invites required for public companies)",
  })
  @ApiResponse({ status: 200, description: "Public companys info returned" })
  @ApiResponse({
    status: 403,
    description: "Company is private",
    type: ErrorResponse,
  })
  @ApiResponse({
    status: 404,
  ...
  async getPublicInfo(@Param("id") companyId: string) {
    ...
    if (!company) {
      this.logger.default(
        `Get company public info failed: company not found - company: ${companyId}`,
      );
    ...
    if (!company.isPublic) {
      this.logger.default(
        `Get company public info failed: company is private - company: ${companyId}`,
      );
    ...
    if (ownerMemberships.length > 0) {
      ...
      const primaryOwnerUser = await this.userRepository.findById(
        primaryOwner.userId,
      );
  ...
  @ApiOperation({
    summary: "Update companys (name/logo/description/is_public)",
  })
  ...
  async remove(@CurrentUser() user: any, @Param("id") companyId: string) {
