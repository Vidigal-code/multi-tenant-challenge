...
export class WsDomainEventsBridgeService implements DomainEventsService {
     ...
     */
    private async handleMembershipJoined(payload: any, eventName: string): Promise<void> {
        if (payload?.userId) {
     ...
     * @param eventName - Name of the domain event
     */
    private async handleMembershipLeft(payload: any, eventName: string): Promise<void> {
        if (Array.isArray(payload?.notifiedUserIds) && payload.notifiedUserIds.length > 0) {
            for (const userId of payload.notifiedUserIds) {
                await this.emitToUserIfAllowed(userId, RT_EVENT.MEMBER_LEFT, payload, eventName);
            }
        } else if (payload?.userId) {
            await this.emitToUserIfAllowed(payload.userId, RT_EVENT.MEMBER_LEFT, payload, eventName);
        }
        if (payload?.companyId) {
     ...
     * @param eventName - Name of the domain event
     */
    private async handleNotificationCreated(payload: any, eventName: string): Promise<void> {
     ...
     * @param eventName - Name of the domain event
     */
    private async handleInviteCreated(payload: any, eventName: string): Promise<void> {
        if (payload?.recipientUserId || payload?.invitedUserId || payload?.email) {
            if (payload.recipientUserId) {
                await this.emitToUserIfAllowed(payload.recipientUserId, RT_EVENT.NOTIFICATION_CREATED, payload, eventName);
            }
        }
        if (payload?.companyId) {
            this.gateway.emitToCompany(payload.companyId, RT_EVENT.NOTIFICATION_CREATED, payload);
     ...
     * @param eventName - Name of the domain event
     */
    private async handleInviteAccepted(payload: any, eventName: string): Promise<void> {
        if (payload?.inviterId) {
            await this.emitToUserIfAllowed(payload.inviterId, RT_EVENT.NOTIFICATION_CREATED, payload, eventName);
        }
        if (payload?.companyId) {
            this.gateway.emitToCompany(payload.companyId, RT_EVENT.NOTIFICATION_CREATED, payload);
        }
     ...
     * @param eventName - Name of the domain event
     */
    private async handleInviteRejected(payload: any, eventName: string): Promise<void> {
        if (payload?.inviterId) {
            await this.emitToUserIfAllowed(payload.inviterId, RT_EVENT.INVITE_REJECTED, payload, eventName);
        }
        if (payload?.companyId) {
     ...
     * @param eventName - Name of the domain event
     */
    private async handleFriendRequestSent(payload: any, eventName: string): Promise<void> {
     ...
     * @param eventName - Name of the domain event
     */
    private async handleFriendRequestAccepted(payload: any, eventName: string): Promise<void> {
     ...
     * @param eventName - Name of the domain event
     */
    private async handleFriendRequestRejectedOrRemoved(payload: any, eventName: string): Promise<void> {
        if (payload?.requesterId) {
            await this.emitToUserIfAllowed(
                payload.requesterId,
