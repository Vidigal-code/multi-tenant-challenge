...
export class WorkersJwsGuard implements CanActivate {
  ...
  constructor(
    ...
    this.algorithm =
      this.configService.get<string>("app.worker.jwt.algorithm") || "ES256";
    this.publicKey = this.configService.get<string>("app.worker.jwt.publicKey");
    this.cookieName =
      this.configService.get<string>("app.worker.jwt.cookieName") || "session";
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      throw new UnauthorizedException(
        `Invalid JWT token: ${error?.message || String(error)}`,
      );
    }
  }
