...
export const appConfig = registerAs("app", () => ({
  ...
  logging: {
    ...
    rabbitmq:
      (process.env.RABBITMQ_LOGGING ?? "false").toLowerCase() === "true",
    websocket:
      (process.env.WEBSOCKET_LOGGING ?? "false").toLowerCase() === "true",
  },
  worker: {
    capacitySharingFactor: parseInt(
      process.env.WORKER_CAPACITY_SHARING_FACTOR ?? "256",
      10,
    ),
    jwt: {
