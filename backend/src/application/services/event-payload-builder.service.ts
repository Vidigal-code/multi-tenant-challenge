...
export class EventPayloadBuilderService {
    ...
    async build(input: EventPayloadBuilderInput): Promise<StandardizedEventPayload> {
        ...
        await this.enrichReceiverInfo(payload, input.receiverId, input.receiverEmail);
     ...
     */
    private buildBasePayload(input: EventPayloadBuilderInput): StandardizedEventPayload {
        return {
     ...
     */
    private async enrichSenderInfo(payload: StandardizedEventPayload, senderId?: string | null): Promise<void> {
        if (!senderId) return;
     ...
     */
    private async enrichCompanyInfo(payload: StandardizedEventPayload, companyId?: string | null): Promise<void> {
        if (!companyId) return;
