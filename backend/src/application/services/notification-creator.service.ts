...
export class NotificationCreatorService {
        ...
        try {
            switch (eventName) {
                ...
                case 'invites.created':
                    this.logger.notificationCreator(`Handling invite.created, inviteId=${payload?.inviteId}, invitedEmail=${payload?.invitedEmail || payload?.receiverEmail || payload?.receiver?.email}`);
     ...
     */
    private async getPrimaryOwnerInfo(members: any[]): Promise<{ name?: string; email?: string }> {
     ...
     */
    private async extractSenderInfo(payload: any, senderId?: string | null): Promise<any | null> {
        if (payload?.sender) {
     ...
     */
    private async extractReceiverInfo(payload: any, receiverId?: string | null): Promise<any | null> {
        if (payload?.receiver) {
     ...
     */
    private buildSenderMeta(sender: any, senderId?: string): {
        id: string;
        name: string;
        email: string;
    } {
    ...
    private buildCompanyMeta(
        ...
        return {
            ...
            description: companyData?.description || companyInfo.companyDescription || null,
     ...
     */
    private buildBaseMeta(kind: string, channel: string, payload: any): {
        kind: string;
        channel: string;
        eventId: string;
        timestamp: string;
    ...
    private async createAndEmitNotification(
        ...
        await this.emitNotificationCreated(notification, companyId, recipientUserId, senderUserId);
        return notification;
    }
    ...
    private async handleNotificationSent(payload: any): Promise<void> {
        ...
        const companyConfig = this.buildCompanyMessageConfig(companyData, companyInfo, finalCompanyId || '');
        ...
        const meta = {
            ...this.buildBaseMeta('notification.sent', finalCompanyId ? 'company' : 'user', payload),
            sender: sender ? this.buildSenderMeta(sender, senderUserId) : undefined,
            company: (companyData || companyInfo.companyId) ? this.buildCompanyMeta(companyData, companyInfo, finalCompanyId || '') : undefined,
        };
        ...
        await this.createAndEmitNotification({
            ...
            body: bodyText,
            meta,
        }, finalCompanyId, receiverId, senderUserId || sender?.id || null);
    }
    ...
    private async handleFriendRemoved(payload: any): Promise<void> {
        ...
        const receiverId = payload?.receiver?.id || 
            (payload?.requesterId && payload?.addresseeId 
                ? (senderId === payload.requesterId ? payload.addresseeId : payload.requesterId) 
                : null);
        if (!senderId || !receiverId) return;
    ...
    private async handleInviteCreated(payload: any): Promise<void> {
        ...
        const company = payload?.company || await this.companyRepo.findById(companyId);
        ...
        const companyConfig = this.buildCompanyMessageConfig(company, companyInfo, companyId);
        ...
        const notification = await this.createAndEmitNotification({
            ...
            body: bodyText,
            meta,
        }, companyId, recipient.id, inviter.id || payload.inviterId);
    ...
    private async handleInviteAccepted(payload: any): Promise<void> {
        ...
        const companyInfo = await this.getCompanyInfo(companyId);
        const companyConfig = this.buildCompanyMessageConfig(companyData, companyInfo, companyId);
    ...
    private async handleRoleChanged(payload: any): Promise<void> {
        ...
        await this.createAndEmitNotification({
            ...
            body: bodyText,
            meta,
        }, companyId, receiverId, senderId || receiverId);
    }
