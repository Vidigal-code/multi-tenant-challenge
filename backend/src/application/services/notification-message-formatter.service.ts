...
export class NotificationMessageFormatterService {
    ...
    formatBody(config: NotificationMessageConfig): string {
        ...
        if (recipientName) {
            bodyParts.push(`Recipient: ${recipientName}${recipientEmail ? ` (${recipientEmail})` : ''}`);
        }
        ...
        if (companyName) {
            ...
            if (companyOwnerName) {
                bodyParts.push(`Primary Owner: ${companyOwnerName}${companyOwnerEmail ? ` (${companyOwnerEmail})` : ''}`);
            }
            if (companyCreatedAt) {
                bodyParts.push(`Created At: ${new Date(companyCreatedAt).toISOString()}`);
            }
