...
export class RealtimeEventDefinitionDto {
    @ApiProperty({example: 'notification.created'}) event!: string;
    @ApiProperty({description: 'JSON schema fields description'}) description!: string;
    @ApiProperty({type: Object, description: 'Payload shape example'}) example!: any;
}

export class RealtimeEventsCatalogResponseDto {
    @ApiProperty({type: [RealtimeEventDefinitionDto]}) events!: RealtimeEventDefinitionDto[];
}