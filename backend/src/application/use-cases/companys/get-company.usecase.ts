...
export class GetCompanyUseCase {
  ...
  async execute(input: GetCompanyInput) {
    ...
    if (!company) {
      this.logger.default(
        `Get company failed: company not found - company: ${input.companyId}, user: ${input.userId}`,
      );
    ...
    if (!company.isPublic) {
      ...
      if (!membership) {
        this.logger.default(
          `Get company failed: user is not a member - company: ${input.companyId}, user: ${input.userId}`,
        );
