...
export class TransferOwnershipUseCase {
  ...
  async execute(input: TransferOwnershipInput) {
    ...
    if (!requesterMembership || requesterMembership.role !== Role.OWNER) {
      this.logger.default(
        `Transfer ownership failed: only owner can transfer - requester: ${input.requesterId}, company: ${input.companyId}`,
      );
    ...
    if (!newOwnerMembership) {
      this.logger.default(
        `Transfer ownership failed: new owner is not a member - newOwner: ${input.newOwnerId}, company: ${input.companyId}`,
      );
    ...
    if (input.requesterId === input.newOwnerId) {
      this.logger.default(
        `Transfer ownership failed: cannot transfer to self - requester: ${input.requesterId}, company: ${input.companyId}`,
      );
      throw new ApplicationError(ErrorCode.CANNOT_TRANSFER_TO_SELF);
    }

    await (this.memberships as any).updateRole?.(
