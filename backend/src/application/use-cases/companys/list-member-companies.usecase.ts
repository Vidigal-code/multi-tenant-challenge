...
export class ListMemberCompaniesUseCase {
  ...
  async execute(
    input: ListMemberCompaniesInput,
  ): Promise<ListMemberCompaniesResult> {
    ...
    for (const membership of userMemberships) {
      ...
      if (company) {
        ...
        const primaryOwner = ownerMembershipsForCompany[0];
        const primaryOwnerUser = primaryOwner
          ? await this.users.findById(primaryOwner.userId)
          : null;
