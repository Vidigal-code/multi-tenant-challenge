...
export class InviteUserUseCase {
  ...
  async execute(input: InviteUserInput): Promise<{ invite: Invite }> {
    ...
    if (!membership) {
      this.logger.default(
        `Invite failed: user is not a member - user: ${input.inviterUserId}, company: ${input.companyId}`,
      );
    ...
    if (membership.role === Role.MEMBER) {
      this.logger.default(
        `Invite failed: insufficient role - user: ${input.inviterUserId}, company: ${input.companyId}, role: ${membership.role}`,
      );
    ...
    if (input.role === Role.OWNER && membership.role !== Role.OWNER) {
      this.logger.default(
        `Invite failed: only owner can invite owner - user: ${input.inviterUserId}, company: ${input.companyId}`,
      );
    ...
    if (inviterUser && inviterUser.email.toString().toLowerCase() === email) {
      this.logger.default(
        `Invite failed: cannot invite self - user: ${input.inviterUserId}, email: ${email}`,
      );
      throw new ApplicationError(ErrorCode.CANNOT_INVITE_SELF);
    }

    const requireExisting =
      (this.config?.get("app.invite.requireExistingUser") as boolean) ?? true;
    let invitedUser = null;
    if (requireExisting) {
      const exists = await this.emailValidation.exists(email);
      if (!exists) {
        this.logger.default(
          `Invite failed: user not found - email: ${email}, company: ${input.companyId}`,
        );
    ...
    if (invitedUser) {
      ...
      if (existingMember) {
        this.logger.default(
          `Invite failed: user is already a member - user: ${invitedUser.id}, company: ${input.companyId}`,
        );
