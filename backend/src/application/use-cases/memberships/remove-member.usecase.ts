...
export class RemoveMemberUseCase {
  ...
  async execute(input: RemoveMemberInput) {
    ...
    if (!requesterMembership) {
      this.logger.default(
        `Remove member failed: requester is not a member - requester: ${input.requesterId}, company: ${input.companyId}`,
      );
    ...
    if (requesterMembership.role === Role.MEMBER) {
      this.logger.default(
        `Remove member failed: insufficient role - requester: ${input.requesterId}, company: ${input.companyId}, role: ${requesterMembership.role}`,
      );
    ...
    if (!targetMembership) {
      this.logger.default(
        `Remove member failed: target is not a member - target: ${input.targetUserId}, company: ${input.companyId}`,
      );
    ...
    if (targetMembership.role === Role.OWNER) {
      ...
      if (ownerCount <= 1) {
        this.logger.default(
          `Remove member failed: last owner cannot be removed - owner: ${input.targetUserId}, company: ${input.companyId}`,
        );
    ...
    if (input.requesterId === input.targetUserId) {
      this.logger.default(
        `Remove member failed: forbidden action - user cannot remove self - user: ${input.targetUserId}, company: ${input.companyId}`,
      );
      throw new ApplicationError(ErrorCode.FORBIDDEN_ACTION);
    }

    const allMembers = await this.membershipRepository.listByCompany(
      input.companyId,
    );
    ...
    const isRequesterPrimaryOwner =
      primaryOwner && primaryOwner.userId === input.requesterId;
    ...
    if (!isRequesterPrimaryOwner) {
      ...
      if (!allowed) {
        this.logger.default(
          `Remove member failed: forbidden action - requester: ${input.requesterId}, target: ${input.targetUserId}, company: ${input.companyId}`,
        );
        throw new ApplicationError(ErrorCode.FORBIDDEN_ACTION);
      }
    }
