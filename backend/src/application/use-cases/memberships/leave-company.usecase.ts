...
export class LeaveCompanyUseCase {
  ...
  async execute(input: LeaveCompanyInput) {
    ...
    if (!membership) {
      this.logger.default(
        `Leave company failed: user is not a member - user: ${input.userId}, company: ${input.companyId}`,
      );
      throw new ApplicationError(ErrorCode.NOT_A_MEMBER);
    }
    if (membership.role === Role.OWNER) {
      this.logger.default(
        `Leave company failed: owner must transfer before leave - user: ${input.userId}, company: ${input.companyId}`,
      );
    ...
    const leadershipMembers = (
      await this.memberships.listByCompany(input.companyId)
    )
      .filter(
        (member) =>
