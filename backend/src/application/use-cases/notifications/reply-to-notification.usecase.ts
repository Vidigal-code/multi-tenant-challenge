...
export class ReplyToNotificationUseCase {
  ...
  constructor(
    ...
    this.logger = new LoggerService(
      ReplyToNotificationUseCase.name,
      configService,
    );
  }
  ...
  async execute(input: ReplyToNotificationInput) {
    if (!input.replyBody || !input.replyBody.trim()) {
      this.logger.default(
        `Reply to notification failed: reply body is required - notification: ${input.notificationId}, user: ${input.userId}`,
      );
      throw new ApplicationError(ErrorCode.MISSING_USER_DATA);
    }

    const originalNotification = await this.notificationRepo.findById(
      input.notificationId,
    );
    if (!originalNotification) {
      this.logger.default(
        `Reply to notification failed: notification not found - notification: ${input.notificationId}, user: ${input.userId}`,
      );
    ...
    if (!canReply) {
      this.logger.default(
        `Reply to notification failed: cannot reply - notification: ${input.notificationId}, user: ${input.userId}, sender: ${originalNotification.senderUserId}, recipient: ${originalNotification.recipientUserId}`,
      );
    ...
    if (!replier) {
      this.logger.default(
        `Reply to notification failed: user not found - user: ${input.userId}`,
      );
