...
export class SendFriendRequestUseCase {
    ...
    async execute(input: SendFriendRequestInput) {
        ...
        if (!requester) {
            this.logger.default(`Friend request failed: requester not found - user: ${input.requesterId}`);
            throw new ApplicationError(ErrorCode.USER_NOT_FOUND);
        }

        if (requester.email.toString() === input.addresseeEmail.toLowerCase().trim()) {
            this.logger.default(`Friend request failed: cannot add yourself - user: ${input.requesterId}`);
        ...
        if (!addressee) {
            this.logger.default(`Friend request failed: addressee not found - email: ${input.addresseeEmail}`);
            throw new ApplicationError(ErrorCode.USER_NOT_FOUND);
        }
        ...
        if (existingFriendship) {
            if (existingFriendship.status === FriendshipStatus.ACCEPTED) {
                this.logger.default(`Friend request failed: already friends - requester: ${input.requesterId}, addressee: ${addressee.id}`);
                throw new ApplicationError(ErrorCode.ALREADY_FRIENDS);
            }
            if (existingFriendship.status === FriendshipStatus.PENDING) {
                this.logger.default(`Friend request failed: request already sent - requester: ${input.requesterId}, addressee: ${addressee.id}`);
                throw new ApplicationError(ErrorCode.FRIEND_REQUEST_ALREADY_SENT);
            }
            if (existingFriendship.status === FriendshipStatus.BLOCKED) {
                this.logger.default(`Friend request failed: user blocked - requester: ${input.requesterId}, addressee: ${addressee.id}`);
