...
export class DeleteAccountUseCase {
  ...
  async execute(input: DeleteAccountInput) {
    ...
    if (!userToDelete) {
      this.logger.default(
        `Delete account failed: user not found - user: ${input.userId}`,
      );
    ...
    while (hasMore) {
      ...
      hasMore =
        ...
        allPrimaryOwnerCompanies.length < totalCompanies;
      currentPage++;
    ...
    for (const membership of userMemberships) {
      ...
      if (membership.role === Role.OWNER) {
        ...
        if (ownerCount <= 1) {
          this.logger.default(
            `Delete account failed: cannot delete last owner - user: ${input.userId}, company: ${membership.companyId}`,
          );
    ...
    await (this.prisma as any).notification.deleteMany({
      where: {
        OR: [{ senderUserId: input.userId }, { recipientUserId: input.userId }],
      },
    });
