...
describe("LeaveCompanyUseCase", () => {
    it("notifies owners and admins when a member leaves", async () => {
        ...
        await memberships.create({userId: "owner-1", companyId: "company-1", role: Role.OWNER});
        await memberships.create({userId: "admin-1", companyId: "company-1", role: Role.ADMIN});
        await memberships.create({userId: "member-1", companyId: "company-1", role: Role.MEMBER});

        const useCase = new LeaveCompanyUseCase(memberships as any, events);

        const result = await useCase.execute({userId: "member-1", companyId: "company-1"});
        ...
        expect(await memberships.findByUserAndCompany("member-1", "company-1")).toBeNull();
    });
    ...
    it("prevents owners from leaving without transferring ownership", async () => {
        ...
        const error = await useCase.execute({userId: "owner-1", companyId: "company-1"}).catch(e => e);
