...
describe("SendNotificationUseCase", () => {
    it("broadcasts to all company members when no contacts are provided", async () => {
        ...
        await memberships.create({userId: owner.id, companyId: "company-1", role: Role.OWNER});
        await memberships.create({userId: admin.id, companyId: "company-1", role: Role.ADMIN});
        await memberships.create({userId: member.id, companyId: "company-1", role: Role.MEMBER});
        ...
        expect(events.events[0].name).toBe("notification.sent");
    ...
    it("validates contact list and only sends to friends or members", async () => {
        ...
        const statuses = result.validationResults.reduce<Record<string, string>>((map, entry) => {
