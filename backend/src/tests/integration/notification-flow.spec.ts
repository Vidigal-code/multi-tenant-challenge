    ...
    async listByUser(filters: {
        ...
        const filtered = this.items.filter((n) => {
            if (filters.read !== undefined) {
                return (
                    (n.recipientUserId === filters.userId || n.senderUserId === filters.userId) &&
                    n.read === filters.read
                );
            }
            return n.recipientUserId === filters.userId || n.senderUserId === filters.userId;
        });
...
describe("Notification Flow Integration", () => {
    ...
    it("complete flow: create company -> send notification -> list -> read -> reply -> delete", async () => {
        ...
        const listNotifications = new ListNotificationsUseCase(notificationRepo as any);
        ...
        const markRead = new MarkNotificationReadUseCase(notificationRepo as any, domainEvents as any);
        ...
        const readNotification = await notificationRepo.findById(String(notifications.data[0].id));
        ...
        const deleteNotification = new DeleteNotificationUseCase(notificationRepo as any);
        ...
        const deletedNotification = await notificationRepo.findById(String(notifications.data[0].id));
        expect(deletedNotification).toBeNull();
    });

    it("send notification to all company members", async () => {
        const owner = await userRepo.create({
            email: "owner@test.com",
            name: "Owner",
            passwordHash: await hashing.hash("password123"),
