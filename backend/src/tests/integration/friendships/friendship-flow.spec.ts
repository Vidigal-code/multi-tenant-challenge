...
class InMemoryFriendshipRepository {
  ...
  async listByUser(filters: {
    ...
    let filtered = this.items.filter(
      (f) =>
        f.requesterId === filters.userId || f.addresseeId === filters.userId,
    );
    if (filters.status) {
...
describe("Friendship Flow Integration", () => {
  ...
  it("complete flow: signup -> send request -> accept -> send message -> delete", async () => {
    ...
    const acceptedFriendship = await friendshipRepo.findByUsers(
      user1.id,
      user2.id,
    );
    ...
    const deletedFriendship = await friendshipRepo.findByUsers(
      user1.id,
      user2.id,
    );
  ...
  it("send request -> reject flow", async () => {
    ...
    const rejectedFriendship = await friendshipRepo.findByUsers(
      user1.id,
      user2.id,
    );
