...
describe("Auth/Select Company Flow (integration pseudo)", () => {
    it("returns NOT_A_MEMBER when selecting companys without memberships", async () => {
        ...
        const error = await usecase.execute({userId: user.id, companyId: "non-member-companys"}).catch(e => e);
