...
describe("Company Lifecycle Integration", () => {
    ...
    it("complete flow: create company -> invite member -> accept -> manage", async () => {
        ...
        const ownerMembership = await membershipRepo.findByUserAndCompany(owner.id, company.id);
        ...
        const memberMembership = await membershipRepo.findByUserAndCompany(member.id, company.id);
        ...
        const selectCompany = new SelectCompanyUseCase(membershipRepo as any, userRepo as any);
        ...
        const updatedMembership = await membershipRepo.findByUserAndCompany(member.id, company.id);
