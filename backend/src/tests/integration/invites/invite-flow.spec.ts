...
import { RejectInviteUseCase } from "@application/use-cases/memberships/reject-invite.usecase";
import {
...
describe("Invite Flow Integration", () => {
  ...
  it("complete flow: signup -> create companys -> invites -> accept", async () => {
    ...
    const membership = await membershipRepo.findByUserAndCompany(
      result.user.id,
      company.id,
    );
  ...
  it("cannot invites existing member", async () => {
    const owner = await userRepo.create({
      email: "owner@test.com",
      name: "Owner",
      passwordHash: await hashing.hash("password123"),
  ...
  it("reuses pending invites for same email and companys", async () => {
    const owner = await userRepo.create({
      email: "owner@test.com",
      name: "Owner",
      passwordHash: await hashing.hash("password123"),
