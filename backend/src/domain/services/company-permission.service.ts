...
export class CompanyPermissionService {
    static canModify(requesterRole: Role, targetRole: Role | null, action:
    CompanyAction, isSelf: boolean = false): boolean {
    ...
    static canRemoveLastOwner(requesterRole: Role, targetRole: Role, ownerCount: number): boolean {
        if (targetRole !== Role.OWNER) return true;
