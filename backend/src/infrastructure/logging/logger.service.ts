...
export class LoggerService {
  ...
  constructor(
    ...
    if (configService) {
      this.rabbitmqLogging =
        configService.get<boolean>("app.logging.rabbitmq", false) ||
        process.env.RABBITMQ_LOGGING === "true";
      this.websocketLogging =
        configService.get<boolean>("app.logging.websocket", false) ||
        process.env.WEBSOCKET_LOGGING === "true";
      this.defaultLogging =
        configService.get<boolean>("app.logging.enabled", true) &&
  ...
  notificationCreator(message: string, ...args: any[]): void {
    if (this.defaultLogging) {
      console.log(
        `${this.PINK}[NotificationCreator] ${message}${this.RESET}`,
        ...args,
      );
    }
  }
