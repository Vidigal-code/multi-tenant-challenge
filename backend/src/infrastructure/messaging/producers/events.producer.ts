...
export class EventsProducer {
  ...
  async emitGenericEvent(payload: Record<string, unknown>): Promise<void> {
    try {
      ...
      this.logger.rabbitmq(
        `Generic event emitted successfully to: ${EVENTS_QUEUE}`,
      );
    } catch (error: any) {
      if (error?.code === 406) {
        this.logger.rabbitmq(
          `Queue ${EVENTS_QUEUE} exists with different config. Attempting to send anyway...`,
        );
        try {
          const serializedPayload = serializeBigInt(payload);
          await this.rabbitmqService.sendToQueue(
          ...
          this.logger.rabbitmq(
            `Failed to send generic event after config mismatch: ${(sendError as Error)?.message}`,
          );
        }
      } else {
        this.logger.error(
          `Failed to emit generic event: ${error?.message || String(error)}`,
        );
      }
    }
  }
  ...
  async emitMemberEvent(payload: Record<string, unknown>): Promise<void> {
    try {
      this.logger.rabbitmq(
        `Emitting member event to queue: ${MEMBERS_EVENTS_QUEUE}`,
      );
      await this.rabbitmqService.assertEventQueue(
      ...
      this.logger.rabbitmq(
        `Member event emitted successfully to: ${MEMBERS_EVENTS_QUEUE}`,
      );
    } catch (error: any) {
      if (error?.code === 406) {
        this.logger.rabbitmq(
          `Queue ${MEMBERS_EVENTS_QUEUE} exists with different config. Attempting to send anyway...`,
        );
        try {
          const serializedPayload = serializeBigInt(payload);
          await this.rabbitmqService.sendToQueue(
          ...
          this.logger.rabbitmq(
            `Failed to send member event after config mismatch: ${(sendError as Error)?.message}`,
          );
        }
      } else {
