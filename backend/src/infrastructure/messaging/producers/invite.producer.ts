...
export class InviteProducer {
  ...
  async emitInviteCreated(payload: Record<string, unknown>): Promise<void> {
    try {
      this.logger.rabbitmq(
        `Emitting invite created event to queue: ${INVITE_QUEUE}`,
      );
      ...
      this.logger.rabbitmq(
        `Invite created event emitted successfully to: ${INVITE_QUEUE}`,
      );
    } catch (error) {
      this.logger.rabbitmq(
        `Failed to emit invite event: ${(error as Error)?.message}`,
      );
    }
  }
  ...
  async emitInviteEvent(payload: Record<string, unknown>): Promise<void> {
    try {
      this.logger.rabbitmq(
        `Emitting invite event to queue: ${INVITES_EVENTS_QUEUE}`,
      );
      ...
      this.logger.rabbitmq(
        `Invite event emitted successfully to: ${INVITES_EVENTS_QUEUE}`,
      );
    } catch (error: any) {
      if (error?.code === 406) {
        this.logger.rabbitmq(
          `Queue ${INVITES_EVENTS_QUEUE} exists with different config. Attempting to send anyway...`,
        );
        try {
          const serializedPayload = serializeBigInt(payload);
          await this.rabbitmqService.sendToQueue(
          ...
          this.logger.rabbitmq(
            `Failed to send invite event after config mismatch: ${(sendError as Error)?.message}`,
          );
        }
      } else {
