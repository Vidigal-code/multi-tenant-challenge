...
export class RabbitMQService implements OnModuleInit, OnModuleDestroy {
  ...
  async sendToQueue(
    ...
    this.logger.rabbitmq(
      `Sending message to queue: ${queue}, size: ${messageSize} bytes`,
    );
  ...
  async assertQueueWithOptions(queue: string, options: any): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        this.logger.rabbitmq(
          `Queue ${queue} exists with different options (PRECONDITION_FAILED).`,
        );
        this.logger.rabbitmq(
          `The queue has different DLQ configuration. Messages in the queue will be preserved.`,
        );
        this.logger.rabbitmq(
          `To fix: Stop all workers, run 'npm run clean:queues', then restart with 'npm run start:all'`,
        );
        this.logger.rabbitmq(
          `Or delete the queue manually from RabbitMQ Management UI: http://localhost:15672`,
        );
        throw new Error(
          `Queue ${queue} exists with different options. Stop all workers, run 'npm run clean:queues', and restart. Error: ${error?.message || String(error)}`,
        );
      } else {
        throw error;
      }
      ...
      if (error?.code === 404) {
        ...
        try {
          await this.channel.deleteQueue(
            queue,
            options || { ifUnused: true, ifEmpty: true },
          );
          this.logger.rabbitmq(
            `Queue deleted successfully with restrictions: ${queue}`,
          );
        } catch (retryError: any) {
          this.logger.rabbitmq(
            `Warning: Could not delete queue ${queue}: ${retryError?.message || String(retryError)}. Continuing anyway.`,
          );
        }
      } else {
        throw error;
  ...
  async assertEventQueue(queue: string, dlq: string): Promise<void> {
    this.logger.rabbitmq(
      `Declarando fila de eventos com DLQ: ${queue} -> ${dlq}`,
    );
  ...
  private async connectWithRetry(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        this.logger.rabbitmq(
          `RabbitMQ connection failed: ${message} (attempt ${attempt}/${maxAttempts})`,
        );
