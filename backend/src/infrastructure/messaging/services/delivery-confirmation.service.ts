...
export class DeliveryConfirmationService {
  ...
  constructor(private readonly config: ConfigService) {
    ...
    const url =
      (this.config.get("app.redisUrl") as string) ||
    ...
    this.ttlSeconds = parseInt(
      process.env.DELIVERY_CONFIRMATION_TTL || "60",
      10,
    );

    this.redis.on("error", (err) => {
      this.logger.error(
        `Redis error in DeliveryConfirmationService: ${err.message}`,
      );
    });
  ...
  async storePendingDelivery(
    ...
    this.logger.default(
      `Pending delivery stored: messageId=${messageId}, userId=${metadata.userId}, ttl=${this.ttlSeconds}s`,
    );
  }
  ...
  async confirmDelivery(
    ...
    if (!data) {
      this.logger.default(
        `Delivery confirmation not found (expired or already confirmed): messageId=${messageId}`,
      );
      return null;
    }
    ...
    this.logger.default(
      `Delivery confirmed: messageId=${messageId}, userId=${parsed.metadata?.userId}`,
    );
    return parsed;
  }
  ...
  startAutoCleanup(intervalMs: number = 60000): () => void {
    const interval = setInterval(async () => {
      try {
        ...
        this.logger.error(
          `Error in auto cleanup: ${error?.message || String(error)}`,
        );
      }
    }, intervalMs);
