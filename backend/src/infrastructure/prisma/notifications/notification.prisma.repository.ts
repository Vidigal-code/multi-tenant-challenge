...
export class NotificationPrismaRepository implements NotificationRepository {
  ...
  async create(data: CreateNotificationInput): Promise<Notification> {
    const created = await (this.prisma as any).notification.create({
      data: {
        ...
        recipientsEmails: (data.recipientsEmails == null
          ? undefined
          : data.recipientsEmails) as any,
        title: data.title,
        body: data.body ?? "",
        meta: data.meta ?? {},
      },
  ...
  async listByUser(
    ...
    return {
      data: rows.map((r: any) => this.toDomainWithSender(r)),
      total,
      page,
      pageSize,
    };
  ...
  async findById(id: string | number): Promise<Notification | null> {
    const record = await (this.prisma as any).notification.findUnique({
      where: { id: Number(id) },
    });
