...
} from "@application/ports/email-validation.service";
import {
  USER_REPOSITORY,
  UserRepository,
  ...
  async exists(emailRaw: string): Promise<boolean> {
    ...
    try {
      await this.redis.set(
        ...
        exists ? "1" : "0",
        "EX",
        exists ? this.ttlHit : this.ttlMiss,
      );
      this.logger.default(
        `Email ${email} validated: ${exists ? "exists" : "does not exist"}, cached for ${exists ? this.ttlHit : this.ttlMiss}s`,
      );
    } catch (err) {
